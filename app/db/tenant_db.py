...
# Pfad zum Verzeichnis für Mandantendatenbanken
...
def init_tenant_db(tenant_uuid: str):
    ...
    # Erstelle Tabellen, auch wenn die DB-Datei bereits existiert,
    # um sicherzustellen, dass das Schema aktuell ist (obwohl create_all idempotent ist).
    try:
        create_all_tenant_tables(engine)
        return engine
    except Exception as e:
        raise # Fehler weiterleiten, damit der Aufrufer ihn behandeln kann

def delete_tenant_db_file(tenant_uuid: str) -> bool:
    db_url = get_tenant_db_url(tenant_uuid)
    db_path = db_url.replace("sqlite:///", "") # Pfad aus URL extrahieren
